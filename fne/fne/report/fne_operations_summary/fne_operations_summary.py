...
def execute(filters=None):
	...
	columns = [
		...
		{"label": "PDF échoué",    "fieldname": "pdf_failed","fieldtype": "Int",     "width": 100},
		{"label": "Échoués certif.","fieldname": "failed",   "fieldtype": "Int",     "width": 110},
	...
	raw = frappe.db.sql(
		...
		SELECT
			...
			COUNT(*)                                                      AS submitted,
			SUM(status IN ('CERTIFIED', 'PDF_PENDING', 'PDF_READY', 'PDF_FAILED')) AS certified,
			SUM(status = 'PDF_READY')                                           AS pdf_ready,
			SUM(status = 'PDF_FAILED')                                          AS pdf_failed,
	...
	for r in raw:
		...
		data.append({
			...
			"certified": certified,
			"pdf_ready":  pdf_ready,
			"pdf_failed": int(r.pdf_failed or 0),
	...
	total_pdf_fail = sum(r["pdf_failed"] for r in data)
	...
	summary = [
		...
		{"label": "PDF non récupérés",   "value": total_pdf_fail, "indicator": "orange"},
		{"label": "Échecs certif.",      "value": total_fail,     "indicator": "orange"},
